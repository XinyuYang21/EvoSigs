...
## Step 5: NMF for each type based on specified ranks
  ...
  rank_summary <- read.csv(file="D:/Project/Work/Phd/Evolution Project/GlasgowCancerEvolution/20200609_redesgin_signature/ccfMat_ICGC_20230104/4.rank_summary_test.csv")
  nmf_folder <- paste0(output,"NMF_result/")
  Matrix_folder <- paste0(output,"ccfMatFraction_9types/")
  
  
  # run NMF for each cancer type 
  #nmf_sig_all_plot(input_folder=Matrix_folder,output=nmf_folder,rank_summary=rank_summary,MatType="fraction",unit=30)
  ...
  nmf_sig_plot_type <- function(type,MatType="fraction",input_folder==Matrix_folder,output=nmf_folder,rank,nrun){
    ...
    type = 'BRCA'
    ccfmat_path <- paste0(input_folder,type,"/",dir(type_path)[grep("ccfFractionMatrix_",dir(type_path))])  
    
    output_path = ".~"
  
  # Manually adjust rank from plot  
  EvoSig_generate <- function(ccfMats,rank,unit)
  
  # perform for multiple types
  lapply(ccfMat_list,function(x) 
    output = EvoSig_generate(ccfMat,rank=rank_list,nrun=100)
    saveRDS(output, file = file.path(output_path, paste0(ncol(ccfMat), "_output_", Sys.Date(), ".rds"))))
